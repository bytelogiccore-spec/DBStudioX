...
async fn export_sql(
    ...
    writeln!(writer, "-- Generated by DBStudioX on {}", chrono::Local::now())
        .map_err(|e| AppError::IoError(format!("Write error: {}", e)))?;
        
    writeln!(writer, "BEGIN TRANSACTION;")
        .map_err(|e| AppError::IoError(format!("Write error: {}", e)))?;
        
    let columns_str = result.columns.iter()
        .map(|c| format!("\"{}\"", c))
        .collect::<Vec<_>>()
        .join(", ");
    ...
    for row in result.rows {
        ...
        let insert_sql = format!(
            "INSERT INTO \"{}\" ({}) VALUES ({});",
            table_name, columns_str, values.join(", ")
        );
...
pub async fn copy_table(
    ...
    log::info!("Copying table {} from {} to {} (target: {})", source_table, source_db_id, target_db_id, target_table);

    if source_db_id == target_db_id && source_table == target_table {
        return Err(AppError::BadRequest("Source and target table cannot be identical in the same database.".to_string()));
    }

    let source_db_handle = state.get_db_handle(&source_db_id)
        .ok_or_else(|| AppError::NotFound(format!("Source connection not found: {}", source_db_id)))?;
        
    let target_db_handle = state.get_db_handle(&target_db_id)
        .ok_or_else(|| AppError::NotFound(format!("Target connection not found: {}", target_db_id)))?;
    ...
    let create_sql = {
        ...
        let result = db.query_with_params(query_sql, vec![serde_json::Value::String(source_table.clone())])
            .map_err(|e| AppError::QueryError(format!("Failed to fetch table schema: {:?}", e)))?;
            
        if result.rows.is_empty() {
            return Err(AppError::NotFound(format!("Source table '{}' not found", source_table)));
        }
    ...
    if with_data {
        ...
        let columns_str = source_data.columns.iter()
            .map(|c| format!("\"{}\"", c))
            .collect::<Vec<_>>()
            .join(", ");
