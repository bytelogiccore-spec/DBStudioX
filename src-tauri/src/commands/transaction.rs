...
pub async fn begin_transaction(
    ...
    // Get database handle and execute BEGIN
    let db_handle = state.get_db_handle(&connection_id)
        .ok_or(AppError::NotFound(format!("Connection not found: {}", connection_id)))?;
    ...
    // Drop the lock explicitly (though it would be dropped at end of scope)
    drop(db);
...
pub async fn commit_transaction(
    ...
    let transaction = state.get_transaction(&transaction_id)
        .ok_or(AppError::NotFound(format!("Transaction not found: {}", transaction_id)))?;

    let connection_id = transaction.connection_id;

    // Get database handle and execute COMMIT
    let db_handle = state.get_db_handle(&connection_id)
        .ok_or(AppError::NotFound(format!("Connection not found: {}", connection_id)))?;
...
pub async fn rollback_transaction(
    ...
    // Get database handle and execute ROLLBACK
    let db_handle = state.get_db_handle(&connection_id)
        .ok_or(AppError::NotFound(format!("Connection not found: {}", connection_id)))?;
