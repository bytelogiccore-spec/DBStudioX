...
pub async fn register_built_in_udfs(
    ...
    log::info!(
        "Registering built-in UDFs for connection: {}",
        connection_id
    );
    ...
    db.create_scalar_function(
        ...
        0,
        true,
        |_ctx: &rusqlite::functions::Context| Ok("0.0.1-alpha".to_string()),
    )
...
pub async fn create_user_function(
    ...
    db.query(&test_sql)
        .map_err(|e| AppError::QueryError(format!("Invalid expression: {:?}", e)))?;
            ...
            for i in 0..arg_count {
                let arg_value = match ctx.get_raw(i as usize) {
                    ...
                    rusqlite::types::ValueRef::Text(v) => {
                        format!("'{}'", String::from_utf8_lossy(v).replace("'", "''"))
                    }
...
pub async fn delete_user_function(
    ...
    log::info!(
        "Marking UDF '{}' for deletion in connection: {}",
        function_name,
        connection_id
    );
