...
impl SqlParser {
    ...
    fn detect_sql_injection_patterns(&self, sql: &str) -> Sqlite3Result<()> {
        ...
        if !starts_with_allowed {
            return Err(Sqlite3Error::InvalidSql(
                "Only INSERT, UPDATE, DELETE, SELECT statements are supported for partitioning"
                    .to_string(),
            ));
        }
        ...
        for keyword in &ddl_keywords {
            if sql_upper.contains(keyword) {
                return Err(Sqlite3Error::InvalidSql(format!(
                    "DDL statements are not supported for partitioning: '{}'",
                    keyword.trim()
                )));
            }
        }
        ...
        if sql_upper.matches("SELECT").count() > 1 {
            return Err(Sqlite3Error::InvalidSql(
                "Nested SELECT statements (subqueries) are not supported for partitioning"
                    .to_string(),
            ));
        }
    ...
    fn validate_identifier(&self, identifier: &str) -> Sqlite3Result<()> {
        ...
        let first_char = identifier
            .chars()
            .next()
            .ok_or_else(|| Sqlite3Error::InvalidSql("Invalid identifier: empty".to_string()))?;
    ...
    pub fn parse_insert(&self, sql: &str) -> Sqlite3Result<ParsedStatement> {
        ...
        let table_end = after_into
            .find([' ', '(', '\n', '\r'])
            .ok_or_else(|| Sqlite3Error::InvalidSql("Table name not found".to_string()))?;
        ...
        let (columns, values) = if rest.starts_with('(') {
            let col_end = rest
                .find(')')
                .ok_or_else(|| Sqlite3Error::InvalidSql("Column list not closed".to_string()))?;
            ...
            if !after_cols.to_uppercase().starts_with("VALUES") {
                return Err(Sqlite3Error::InvalidSql(
                    "VALUES keyword not found".to_string(),
                ));
            }
            ...
            let val_end = after_values
                .find(')')
                .ok_or_else(|| Sqlite3Error::InvalidSql("VALUES clause not closed".to_string()))?;
            let val_list = &after_values[1..val_end];
            let values: Vec<String> = val_list
                .split(',')
                .map(|s| s.trim().to_string())
                .filter(|s| !s.is_empty())
                .collect();
    ...
    pub fn parse_update(&self, sql: &str) -> Sqlite3Result<ParsedStatement> {
        ...
        if !rest.to_uppercase().starts_with("SET") {
            return Err(Sqlite3Error::InvalidSql(
                "SET keyword not found".to_string(),
            ));
        }
    ...
    pub fn parse_select(&self, sql: &str) -> Sqlite3Result<ParsedStatement> {
        ...
        let from_pos = sql_upper
            .find(" FROM ")
            .ok_or_else(|| Sqlite3Error::InvalidSql("FROM keyword not found".to_string()))?;
    ...
    pub fn extract_partition_key_value(
        &self,
        statement: &ParsedStatement,
        key_column: &str,
    ) -> Sqlite3Result<String> {
        match statement.statement_type {
            StatementType::Insert => {
                let key_index = statement
                    .columns
                    .iter()
                    .position(|col| col.eq_ignore_ascii_case(key_column))
                    .ok_or_else(|| {
                        Sqlite3Error::PartitionKeyNotFound(format!(
                            "Key '{}' not found in INSERT",
                            key_column
                        ))
                    })?;
                statement.values.get(key_index).cloned().ok_or_else(|| {
                    Sqlite3Error::PartitionKeyNotFound("Value not found".to_string())
                })
            }
            StatementType::Update | StatementType::Delete | StatementType::Select => {
                let where_clause = statement.where_clause.as_ref().ok_or_else(|| {
                    Sqlite3Error::PartitionKeyNotFound("WHERE clause required".to_string())
                })?;
                ...
                if let Some(pos) = where_upper.find(&pattern_upper) {
                    ...
                    Ok(after_eq[..value_end]
                        .trim()
                        .trim_matches(['\'', '"', '`'])
                        .to_string())
                } else {
                    Err(Sqlite3Error::PartitionKeyNotFound(format!(
                        "Key '{}' not found in WHERE",
                        key_column
                    )))
                }
            }
